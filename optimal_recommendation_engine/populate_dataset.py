...
class populate_dataset:
    def __init__(self) -> None:
        ...
        self.country_names = [countries[i]['name'] for i in range(len(countries))]

        self.ethnicities = ['african', 'carribean', 'asian', 'bangladeshi', 
        'canadian', 'chinese', 'filipino', 'french', 'indian', 'indonesian', 
        'japanese', 'latin american', 'american', 'muslim', 'nepalese', 'pakistani', 'persian', 
        'punjabi', 'south asian', 'south east asian', 'thai', 'asian']

        self.genders = ['male', 'female', 'transgender', 'other']

        self.religions = ['christian', 'muslim', 'atheist', 'hindu', 'buddhist', 'indigenous', 'sikhism', 'judaism', 'jainism']
    ...
    def generate_personal_tags(self):
        self.country = []
        self.gender = []
        self.religion = []
        self.ethnicity = []
        ...
        for i in range(self.number_of_students):
            self.country.append(random.choice(self.country_names))
            self.religion.append(random.choice(self.religions))
            self.ethnicity.append(random.choice(self.ethnicities))

        self.gender = random.choices(population=self.genders, weights=[0.4, 0.4, 0.1, 0.1], k=self.number_of_students)
        ...
        complete_details = pd.DataFrame(
            list(
                zip(
                    ...
                    self.salt_values, self.gender, 
                    self.ethnicity, self.religion, self.country,
                    self.faculty, self.club_interests, self.event_interests)))

        complete_details.to_csv(
            '../generated_csv_files/1_student_all_interests.csv', header=["email", "hash_key", "salt", "gender", "ethnicity", "religion", "country", "faculty", "clubs", "events"])
        print("\n\n Process completed!")
